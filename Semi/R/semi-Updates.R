...
mergeNo  <- read.csv("semi.csv")[c(18)]
mergeInc <- read.csv("semi.csv")[c(20)]
insert   <- read.csv("semi.csv")[c(22)]

# =======================================  P L O T   L A Y E R  ============ 

xrange = range(0:50)
yrange = range(0:1250000) 

plot( main ="semi-FT: Updates (insertion and appending) in a forest of 50-node trees" 
    , xrange, yrange
...
lines(lowess(mergeNo + 20000)          # lowess (trees)  
...
legText = c( "Append Trees (no search)", "Append Trees (with search)","Insertion" )           
